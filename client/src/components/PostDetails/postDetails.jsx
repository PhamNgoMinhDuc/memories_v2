...
const postDetails = () => {
  ...
  return (
    <div className={`md:h-screen  h`}>
      <NavBar />
      <div>
        <div className="h-20"></div>
        <div className="w-full h-full form mb-10">
          <div className="flex flex-wrap-reverse h-auto gap-2 md:flex-nowrap md:flex-row">
            <div className="flex-auto w-full md:flex-auto md:w-[50%]">
            ...
            <img
              className="flex-auto w-full md:flex-auto md:w-[50%] h-auto object-cover rounded-[5px]"
      ...
      {recommendedPosts.length === 0 ? (
        ...
        <div className="form">
          <>
            <div>{t("detailPost.like")}</div>
            <div className="flex flex-col md:flex-row gap-2">
              {recommendedPosts.map(({ title, message, name, likes, selectedFile, _id }) => (
                <div className="relative mb-5 overflow-hidden rounded-[10px] " key={_id} onClick={() => openPost(_id)}>
                  <img
