          ...
          <div className=" flex flex-col items-center">
            ...
            {switchBtn ? (
              <div>
                ...
                {errorInfor &&
                  (lng === "en" ? <div className="text-error">{errorInfor.firstName}</div> : errorInfor.firstName === "First name is empty" ? <div className="text-error">Họ rỗng</div> : "")}
                {errorInfor &&
                  (lng === "en" ? <div className="text-error">{errorInfor.lastName}</div> : errorInfor.lastName === "Last name is empty" ? <div className="text-error">Tên rỗng</div> : "")}
                <div className="my-2">
                  <FileBase type="file" multiple={false} onDone={({ base64 }) => setformDataInfor({ ...formDataInfor, avatar: base64 })} />
                </div>
                {errorInfor &&
                  (lng === "en" ? <div className="text-error">{errorInfor.avatar}</div> : errorInfor.avatar === "Avatar is empty" ? <div className="text-error">Hình đại diện rỗng</div> : "")}
              </div>
            ) : (
              <div className="w-full">
                ...
                {errorPasswod &&
                  (lng === "en" ? (
                    <div className="text-error">{errorPasswod.oldPassword}</div>
                  ) : errorPasswod.oldPassword === "Old password is empty" ? (
                    <div className="text-error">Mật khẩu rỗng</div>
                  ) : errorPasswod.oldPassword === "Wrong password" ? (
                ...
                {errorPasswod &&
                  (lng === "en" ? (
                    <div className="text-error">{errorPasswod.newPassword}</div>
                  ) : errorPasswod.newPassword === "New password is empty" ? (
                    <div className="text-error">Mật khẩu mới rỗng</div>
                  ) : (
                    ""
                  ))}
              </div>
